...
pleiotropyh2_cor_computing_single_prune<-function(G,phenotype,h2_vector,h2_vector_mat,Results_full_rg,Results_full_rg_array,sample_rep){
  ...
  message("Start computing initial value...")
  ...
  message("finish computing initial value...")
  ...
  message(paste0("mean_estimate: ",as.numeric(mean_est_a_single)))
  message(paste0("observed_true_estimate: ",as.numeric(true_pleio_cor_test)))
  message(paste0("corrected_weight: ",1))
  message(paste0("difference: ",as.numeric(abs(mean_est_a_single-true_pleio_cor_test)/true_pleio_cor_test)))
  message("Start binary search...")

  while (max(abs(upper_bound_single-lower_bound_single)) > 1e-6) {
    message(paste0("search_round:",t))
    ...
    message(paste0("mean_estimate: ",as.numeric(mean_est_a_single)))
    message(paste0("observed_true_estimate: ",as.numeric(true_pleio_cor_test)))
    message(paste0("corrected_weight: ",as.numeric(ratio_a)))
    message(paste0("difference: ",as.numeric(abs(mean_est_a_single-true_pleio_cor_test)/true_pleio_cor_test)))
